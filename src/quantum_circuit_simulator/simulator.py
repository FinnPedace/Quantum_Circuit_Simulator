...
class StatevectorSimulator:
    def _validate_circuit(self, circuit: QuantumCircuit) -> None:
        ...
        if len(circuit.cregs) > 1:
            raise NotImplementedError("Mehrere Classical Registers werden nicht unterstützt.")
            
        measurement_seen = False # Stellt sicher, dass keine Gates nach Messungen auftreten
        measured_qubits = set() # Hält die Menge der gemessenen Qubits, um Teilmessungen zu erkennen
        ...
        for instruction in circuit.data:
            ...
            raise NotImplementedError(f"Die Operation '{op_name}' ist nicht implementiert undwird nicht unterstützt.")

        if measurement_seen and len(measured_qubits) != circuit.num_qubits: #Alle Qubits müssen gemessen werden, Teilmessungen sind nicht erlaubt
